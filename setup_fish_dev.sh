...
if command -v fish >/dev/null 2>&1; then
  ...
  if [ "$CURR_SHELL" != "/usr/bin/fish" ] && [ "$CURR_SHELL" != "/bin/fish" ]; then
    if [ -x /usr/bin/fish ]; then chsh -s /usr/bin/fish "$USER" || true; fi
    if [ -x /bin/fish ]; then chsh -s /bin/fish "$USER" || true; fi
  fi
fi
